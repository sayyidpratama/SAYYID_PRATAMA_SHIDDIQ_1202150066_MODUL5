...
public class database extends SQLiteOpenHelper {
    ...
    public void readdata(ArrayList<LauncherActivity.ListItem> daftar){

        Cursor cursor = this.getReadableDatabase().rawQuery("select todo, description, priority from "+nama_tabel, null);

        while (cursor.moveToNext()){

            daftar.add(new LauncherActivity.ListItem(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
